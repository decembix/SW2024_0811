...
const PASSWORD_RESET_INTERVAL = 1 * 60 * 60 * 1000; // 1시간
...
router.post('/reset_password_request', async (req, res) => {
    ...
    try {
        ...
        const now = Date.now();
        if (user.lastPasswordReset && (now - user.lastPasswordReset) < PASSWORD_RESET_INTERVAL) {
            const remainingTime = Math.ceil((PASSWORD_RESET_INTERVAL - (now - user.lastPasswordReset)) / (60 * 1000));
            return res.status(400).json({ msg: `비밀번호 재설정 요청은 1시간에 한 번만 가능합니다. ${remainingTime}분 후에 다시 시도하세요.` });
        }
        ...
        await user.save();  // 여기에서 lastPasswordReset이 DB에 저장됨
